...
function_sample <- function(target_id, input_folder, output_folder, core, SNP, quality_filter = 45, target_mut_filter_rate = 0.3) {
    ...
    empty_output <- function() {
        write.table(data.frame(new_read = character()), file = file.path(output_folder, paste0(target_id, ".txt")),
                    col.names = F, quote = F, row.names = F)
        return(-1)
    }

    if(!file.exists(align_file) || file.info(align_file)$size == 0) {
        return(empty_output())
    }
